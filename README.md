...
| a11y snapshot observation channel | Implemented | An optional, read-only second observation channel on the tab a surf run already owns: `surf explore --a11y-snapshot[=required]` or `agents.<name>.observation.a11ySnapshot`. agent-browser (>= 0.35.1) attaches to the same loopback CDP target, one `snapshot -i --json` is written as `a11y-snapshot.v1` under `receipts.dir/<runId>/` at 0600, and the envelope carries the sha256 digest, the `{role, name}` refs map, the role counts and `semanticCoverage` (what the DOM counted against what the tree could name). Off by default; a missing binary, an old version, a non-loopback or unreachable endpoint, an ambiguous tab binding or an empty tree is a typed refusal, never a launched browser |
