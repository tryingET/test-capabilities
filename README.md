...
| a11y snapshot observation channel | Implemented | An optional, read-only second observation channel on the tab a surf run already owns: `surf explore --a11y-snapshot[=required]` or `agents.<name>.observation.a11ySnapshot`. Chromium's own accessibility tree (`Accessibility.getFullAXTree` over the loopback CDP endpoint, out-of-process frames through their own sessions, shadow DOM included) is rendered as structure and written as `a11y-snapshot.v1` under `receipts.dir/<runId>/` at 0600; the envelope carries the sha256 digest, the `{role, name}` refs map, the role counts and `semanticCoverage`. `openA11yLiveView` gives `evaluateA11yAssertion` a reader for `visible`, `text` and `attr`. Off by default; a non-loopback or unreachable endpoint, an ambiguous tab or an empty tree is a typed refusal. Chosen by a measured series against surf's own tree (`docs/project/2026-09-26-a11y-producer-switch.md`) |
| frame root cause | Implemented | `surf explore --ready-selector <css>` (or `agents.<name>.readySelector` on a surf agent in a `test` run) puts a selector into the readiness gate, and a selector the gate cannot reach gets one read-only `surf frame.diagnose` in the same owned tab. The page's frames become typed topology and the answer is a determination - `excluded`, `confirmed`, `suspected`, `undetermined`, `unavailable` - carried on the probe and the finding. `confirmed` requires the test author's `--frame-hint 'urlPrefix=...'` or `'selector=...'` (`agents.<name>.frameHint` in a `test` run) resolving to exactly one reachable candidate; it is the only value that files `frame_boundary`, and everything short of `excluded` is reported as `browser_coverage_gap`, a limit of the sensor rather than a fault of the target. The healer refuses a confirmed boundary with the `frame.switch` the repair needs, and caveats a suspected one into review |
| `surf plan` | Implemented | Reads a form in an owned tab and writes a reviewable 0600 plan artifact: resolved selectors, intended values, the one control that may be clicked (or an explicit ambiguous/none), the form-level buttons that never may, a page fingerprint and an RFC 8785 approval token over the plan's content. Read-only; a field that resolves to a button is refused |
| `surf apply` | Implemented | Carries out a plan on a runner whose addressable set is the plan's own fields plus, in submit mode only, the identified control. Dry-run fill by default; `--submit` needs `--confirm-plan <token>` and an origin in `mutation.allowOrigins`; one receipt per act, written before the click, and a submitted plan is never submitted twice |
...
The browser surface is a scope, not an object with authority. Every surf command carries a static effect class the caller cannot override; the run acts only in a tab it created (`owned_tab_required` otherwise); page-side `js` has no class until the caller declares one, and a `read_only` claim is checked against a denylist before any process starts (`read_only_violation`); a read-only attempt whose own evidence shows the page moved forfeits its retry budget (`read_only_violation_observed`); and a browser step whose process reported nothing is `unknown` behind a receipt, never a target fault.

An exit code is no longer a verdict. Every sensor step carries a typed `ResultOutcome` with a basis, and the run composes them into `TestResult.determination { value, basis, reason }` next to `passed` (which is now `determination.value === "verified"`). The values are `verified`, `failed`, `unverified` and `indeterminate`; exit codes stay 0/1, with `unverified` and `indeterminate` mapping to 1 (operator decision D3), and `docs/api/errors.md` carries the mapping. `test --json` gains `determination`, `outcomes`, `bases`, `runId`, `effect` and `mutations`; `surf explore --json` gains `probes[].outcome`, `probes[].frameRootCause` and `pages[].observations`. Every field is additive.

Failures are typed. A framework error carries a registered code: text mode prints `<message> [code]` on stderr, `--json` prints `{"error": {code, message, details}}` on stdout and exits 1, and a mutation refusal lists the receipts that blocked it in `details.receipts` together with the exact `--supersede-receipt <id>` line that resolves it.

### What an existing 0.3.x setup has to change

| If you | You now get | The fix |
|---|---|---|
| import `SurfClient`, `SurfFlowBuilder`, `createNexus`, `NexusOrchestrator`, `NexusConfig` or the default export | a module resolution error: the names are gone from the package root | `Session`/`SurfSession` for the browser (`docs/api/api-surf.md`), `createTestCapabilities` and `TestCapabilitiesOrchestrator` for the rest; `docs/api/exports.generated.md` is the full list the package exports |
| run a CLI target that exits 0 and prints nothing | `empty_result` with basis `no_evidence`, `determination: unverified` and exit 1 - a silent command is not a pass | declare the shape: `agents.<name>.expect.output: empty` (with `empty_marker`/`payload`/`error_envelope` as needed), which records who declared it, or make the target print its evidence |
| run a `bombadil` or `terminal-fuzzer` agent | `mutation_origin_not_allowed` before anything is spawned | name the origin in `mutation.allowOrigins` in the config; there is no flag and no environment variable that adds one |
| run a mutating agent with `receipts.dir` inside `$TMPDIR`, a CI job workspace or a disposable worktree | `mutation_receipts_ephemeral`: an interlock that vanishes with the workspace is not an interlock | point `receipts.dir` at a durable directory, or accept it explicitly with `receipts.ephemeral: true` (recorded in every receipt written under it) |
