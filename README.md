...
| a11y snapshot observation channel | Implemented | An optional, read-only second observation channel on the tab a surf run already owns: `surf explore --a11y-snapshot[=required]` or `agents.<name>.observation.a11ySnapshot`. One surf `page.read --structure --full-page --no-text --nodes` (controls, headings and landmarks across the whole page, deterministic, no diff footer) is written as `a11y-snapshot.v1` under `receipts.dir/<runId>/` at 0600, and the envelope carries the sha256 digest, the `{role, name}` refs map, the role counts and `semanticCoverage` (what the DOM counted against what the tree could name). Off by default; a surf without `--nodes`, an empty tree or a page that moved is a typed refusal. The producer was agent-browser over CDP until AK #5915; no second tool, CDP endpoint or stray tab remains |
