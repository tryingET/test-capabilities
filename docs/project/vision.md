...
Current supported runtime surfaces include the typed operation kernel, `test`, `quantum`, `heal`, `surf explore`, `cli-tester`, Bombadil-backed exploration, normalized `observation.v1` events from supported agents, finding correlation, correlation-gated diagnostic semantic synthesis/correlation, and deterministic calibrated `root_cause` observations for independent same-component failed-or-errored observed current-run evidence from at least two sensors with same-class agreement. Surf is now the first concrete multi-probe sensory surface: it verifies browser state, DOM shape, and same-origin link discovery before contributing graded user-flow coverage.
...
- Normalized `observation.v1` events from supported agents, diagnostic semantic synthesis/correlation, and first deterministic calibrated `root_cause` observations for independent same-component failed-or-errored observed current-run evidence from at least two sensors with same-class agreement
