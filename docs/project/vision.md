...
Current supported runtime surfaces include the typed operation kernel, `test`, `quantum`, `heal`, `surf explore`, `cli-tester`, Bombadil-backed exploration, normalized `observation.v1` events from supported agents, finding correlation, correlation-gated diagnostic semantic synthesis/correlation, deterministic calibrated `root_cause` observations for independent same-component failed-or-errored observed current-run evidence from at least two sensors with same-class agreement (now including bounded API auth-boundary, API/web network-connectivity, API/CLI/web resource-exhaustion, and API/CLI/web configuration-error classification), and low-calibration non-authoritative `propagation` observations for bounded dependency-topology links after high-calibration component root_causes. Surf is now the first concrete multi-probe sensory surface: it verifies browser state, DOM shape, and same-origin link discovery before contributing graded user-flow coverage. Propagation output is diagnostic linkage, not causal proof, prediction, or repair-order authority.
