...
Current supported runtime surfaces include the typed operation kernel, zero-external `doctor` and `demo` first-run paths, `test`, `quantum`, `heal`, `surf explore`, `cli-tester`, Bombadil-backed exploration, normalized `observation.v1` events from supported agents, finding correlation, correlation-gated diagnostic semantic synthesis/correlation, deterministic calibrated `root_cause` observations for independent same-component failed-or-errored observed current-run evidence from at least two sensors with same-class agreement (now including bounded API auth-boundary, API/web network-connectivity, API/CLI/web resource-exhaustion, and API/CLI/web configuration-error classification), and low-calibration non-authoritative `propagation` observations for bounded dependency-topology links after high-calibration component root_causes. Surf is now the first concrete multi-probe sensory surface: it verifies browser state, DOM shape, and same-origin link discovery before contributing graded user-flow coverage. Propagation output is diagnostic linkage, not causal proof, prediction, or repair-order authority.
...
Dependency-intelligence alignment: `test-capabilities` can be used as a target/behavior-validation consumer for dependency-intelligence review programs because it exposes bounded CLI/API/runtime scenarios, behavior checks, package-level probes, and owner-side validation commands. It does not own dependency-intelligence standards, classification vocabulary, depmodel production, report rendering, vulnerability review, or cross-repo corridor authority. Those remain with runtime-trace-insights, Gardener/static analyzers, dep-diet, dep-viz, dep-redteam, ts-quality evidence, and FCOS control-board coordination.
...
- Normalized `observation.v1` events from supported agents, diagnostic semantic synthesis/correlation, first deterministic calibrated `root_cause` observations for independent same-component failed-or-errored observed current-run evidence from at least two sensors with same-class agreement, and low-calibration non-authoritative `propagation` observations for bounded dependency-topology links with guardrails against causal, predictive, or repair-order overclaims
