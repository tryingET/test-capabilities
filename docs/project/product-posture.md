...
10. Closed (AK #5567, `docs/project/2026-09-23-tableak-greats-adjudication.md`): the a11y channel attributes a leak and never subtracts one. Exactly one `about:blank` from a measured agent-browser version (0.35.1) is `known_producer_stray` and stays evidence; any other page the channel leaves is `unexplained` and refuses a `required` channel with `tab_leak`. The effect declaration now says the channel leaves one page target per session. Still open: the stray accumulates one page per run; filed upstream as vercel-labs/agent-browser#1986 (a fresh `--pin-tab` session opens a tab on attach before `tab <targetId>` binds). Superseded by AK #5915 (`docs/project/2026-09-26-a11y-producer-switch.md`): agent-browser is retired, and the channel reads Chromium's accessibility tree over CDP from the existing page target, which creates no tab; `tabLeak` and `tab_leak` are gone.
11. Closed (AK #5568): `agents.<name>.readySelector` (alias `ready_selector`, surf agents only) forwards a ready selector to the surf agent's nested explore, so a `test` orchestrator run produces an element-reach failure and carries the frame determination onto its finding (`tests/cli_fail_closed_contract.test.mjs`, fake surf). `agents.<name>.frameHint` (AK #5885) is the config twin of `--frame-hint`, so a `test` run can reach `confirmed` and file `frame_boundary`.
12. Closed (AK #5569; adjudicated in `docs/project/2026-09-23-frame-probe-greats-adjudication.md`, measured live in `docs/project/2026-09-23-frame-probe-live-measurement.md`): the in-frame positive probe, opt-in as `surf explore --frame-probe` and `agents.<name>.frameProbe`. Per top-level candidate it runs `frame.switch --index <domIndex>`, one `wait.element`, and `frame.main` inside `Session.inFrame`, a scoped `browser_session` step that always restores the context and closes the tab if it cannot. It may only promote (one hit, every candidate answering, is `confirmed`) and never demotes to `excluded`. Nested candidates are unprobed and block a confirmation, so pages whose candidates nest frames stay `suspected`.
