...
12. Open (AK #5569, deferred until a live frame.js/frame.switch measurement; adjudicated in `docs/project/2026-09-23-frame-probe-greats-adjudication.md`): the in-frame positive probe. The gate rule is decided (a probe may promote to `confirmed`, never demote to `excluded`); `frame.js` cannot reach out-of-process frames, so the probe needs a scoped `frame.switch` step class from the mutation-safety contract. `frame.switch --index` plus a read-only query for the failing selector is what would confirm a frame boundary without the test author's hint; it changes extension state for the tab and is designed together with mutation safety.
