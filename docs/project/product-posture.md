...
10. Closed (AK #5567, `docs/project/2026-09-23-tableak-greats-adjudication.md`): the a11y channel attributes a leak and never subtracts one. Exactly one `about:blank` from a measured agent-browser version (0.35.1) is `known_producer_stray` and stays evidence; any other page the channel leaves is `unexplained` and refuses a `required` channel with `tab_leak`. The effect declaration now says the channel leaves one page target per session. Still open: the stray accumulates one page per run, a fix that belongs upstream in agent-browser.
