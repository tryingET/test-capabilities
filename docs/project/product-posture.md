...
11. Closed (AK #5568): `agents.<name>.readySelector` (alias `ready_selector`, surf agents only) forwards a ready selector to the surf agent's nested explore, so a `test` orchestrator run produces an element-reach failure and carries the frame determination onto its finding (`tests/cli_fail_closed_contract.test.mjs`, fake surf). There is no config twin of `--frame-hint`, so a `test` run stops at `suspected`; `frame_boundary` still needs `surf explore --frame-hint` or `agents.<name>.frameHint` (filed as AK #5885).
