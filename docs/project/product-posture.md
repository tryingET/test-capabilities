...
10. Closed (AK #5567, `docs/project/2026-09-23-tableak-greats-adjudication.md`): the a11y channel attributes a leak and never subtracts one. Exactly one `about:blank` from a measured agent-browser version (0.35.1) is `known_producer_stray` and stays evidence; any other page the channel leaves is `unexplained` and refuses a `required` channel with `tab_leak`. The effect declaration now says the channel leaves one page target per session. Still open: the stray accumulates one page per run; filed upstream as vercel-labs/agent-browser#1986 (a fresh `--pin-tab` session opens a tab on attach before `tab <targetId>` binds).
...
12. Open (AK #5569; adjudicated in `docs/project/2026-09-23-frame-probe-greats-adjudication.md`, measured live in `docs/project/2026-09-23-frame-probe-live-measurement.md`): the in-frame positive probe. The gate rule is decided (a probe may promote to `confirmed`, never demote to `excluded`) and the design is measured: `frame.switch --index <domIndex>` + `wait.element --selector` + `frame.main` per top-level candidate - never `js` after a switch, which reads the main document - inside a scoped switch-with-restore step class the mutation-safety contract does not have yet. `frame.switch --index` plus a read-only query for the failing selector is what would confirm a frame boundary without the test author's hint; it changes extension state for the tab and is designed together with mutation safety.
