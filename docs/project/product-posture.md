...
9. Follow-up for the surf runtime: wire `surf frame.diagnose` into the root-cause workflow as a `Session.observe` step so `selector_or_dom_drift` findings on Surf evidence can attach the iframe/shadow-root diagnosis (plan S8); the `SurfClient` class left the public API in 0.4.0 and the kernel `Session` interface replaces it (plan S6); the migration off the retired `surf-go` fork is documented in `docs/project/2026-09-07-surf-cli-migration-live-run.md`.
