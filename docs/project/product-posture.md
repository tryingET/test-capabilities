...
- `test`, `quantum`, `heal`, and `surf explore` have real operation paths; `heal --dry-run --proposal-output --verification-output` can emit durable proposal and in-memory verification artifacts without mutating files, and apply-mode healing now requires an externally-owned `--checkpoint-ref` before mutating files, with `--proposal-input` available when operators want apply to consume a previously reviewed proposal artifact instead of recomputing proposals.
...
9. Follow-up for the surf runtime: wire `surf frame.diagnose` (exposed today as `SurfClient.diagnoseFrames`) into the root-cause workflow so `selector_or_dom_drift` findings on Surf evidence can attach the iframe/shadow-root diagnosis, and live-verify the remaining `SurfClient` mappings (`read`, `screenshot`, `network`) against the agent browser; the migration off the retired `surf-go` fork is documented in `docs/project/2026-09-07-surf-cli-migration-live-run.md`.
