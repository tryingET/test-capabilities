...
orchestrator agents (`CliTesterAgent`, `SurfAgent`, `BombadilAgent`, `TerminalFuzzerAgent`) and the explore probes
...
Config (strict schema, so the key must be added to `AgentConfigSchema` at `orchestrator.ts:92-102`,
`docs/api/config.md` and `src/core/capabilities.ts`):
...
- `TestOperationSummary` gains `outcomes: Record<OutcomeClass, number>` and `bases: Record<OutcomeBasis, number>`;
  `health` semantics do not change.
