...
- Config `receipts.dir` in `test-capabilities.yaml` (default `.test-capabilities/receipts`, resolved against the config file's directory; the submit-gate packet's `surf.receipts.dir` folds into this one key). The ledger writes `<receipts.dir>/<run_id>/<receipt_id>.json` (mode 0600, through the kernel artifact writer `src/core/artifacts.ts`, lifted from `heal-operation.ts:108-136` and extended with fsync on file and directory, artifact kinds, symlink refusals and `list(kind, filter)`; revised by architecture review: A9). The interlock is per `receipts.dir`: a CI runner with a per-job workspace is not interlocked unless the key points at a persistent path, and the docs say so. Ledger writes are framework bookkeeping, not steps: they carry no effect class and no receipt of their own, exactly as the dry-run artifacts do today. A mutating operation whose `receipts.dir` is not writable refuses before any step with `mutation_receipt_write_failed`; read-only operations never touch it.
...
- Revised by architecture review (A7, A8): the kernel owns one process boundary, `spawnStep(source, argv, { timeoutMs, env })`, which returns the result-classification packet's `RawResult`; `runStep` is the only caller of `spawnStep` for mutating steps, enforced by a contract test that greps `src/` for `spawn(`/`spawnSync(` outside the kernel. Browser steps run over the surf adapter's owned-tab `BrowserSession` (open, gate, `step(EffectStep)`, `observe(name, readOnlyStep)`, close in `finally`); explore, plan, apply, the frame diagnosis and the a11y observer are step lists over that session, so every browser step is an `EffectStep` and yields a `ResultOutcome` without special casing.
