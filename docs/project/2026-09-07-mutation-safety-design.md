...
| `surf-client.ts:903-983` `SurfFlowBuilder.execute` | sequential steps, stop on first failure, no retry, no receipt | re-executing the builder replays every click; revised by architecture review (A3): the builder is deleted outright in the same commit that removes `type({submit})` (submit-gate D8), it is never ledger-wrapped |
...
The quantum runner is a seeded in-memory simulation with no process spawn or network call (`src/quantum/simulator.ts:160-217`, `:632-639`); it is read-only. Operator decision D1 (2026-09-07, adjudication claim 43 fallback): quantum and prediction stay in the runtime as parked, `read_only` code with reason "parked; produces no target evidence"; they must never produce target evidence, never write a `Finding` or `Observation`, and never influence `passed` or the run `determination`; the passport rows say `parked`, README row 94 stops saying Implemented, and they are excluded from the coverage floor.
...
export class MutationLedger {
  // revised by architecture review (A5): the context is minted once per executeCliOperation call
  constructor(context: RunContext /* {runId, startedAt, config, receiptsDir, ledger} */, operationId: OperationId);
  runStep<T>(step: EffectStep<T>): Promise<T>;   // the only caller of the kernel spawnStep for mutating steps (A7)
  receipts(): MutationReceipt[];
  listReceipts(filter: { idempotencyKey?: string; planId?: string; mode?: string }): MutationReceipt[]; // reads receipts.dir (A1, A2)
}
// owned by src/core/result-classification.ts and re-exported here (A7)
...
- Agents in `orchestrator.ts` declare a static class: `cli-tester` read_only (reason: "runs `--help` only; assumed read-only, not verified", rendered in every envelope's `effect.reason`; revised by adjudication: claim 50), `surf` read_only with `browser_session` lifecycle, `bombadil` and `terminal-fuzzer` mutating/target. `test` resolves to the worst class of its enabled agents.
- The surf adapter carries a static command -> class map (`Adapter.effects`), enforced in `Session.step` (formerly stated on `SurfClient.run()`; `SurfClient` is deleted under operator decision D2, revised by adjudication: claim 36). Read-only: `read`, `page.*`, `wait*`, `screenshot`, `tab.list`, `network*`, `console`, `cookie.list`, `frame.list|diagnose`, `extract`, `scroll.*`, `emulate.*`. Runtime-owned `browser_session`: `tab.new`, `tab.close`, `tab.switch`, `window.*`, `frame.switch`, `frame.main` (revised by adjudication: Part 4 P3 note; the frame-root-cause packet's deferred in-frame probe needs this class). Mutating/target: `click`, `type`, `key`, `select`, `locate.* --action`, `do`, `go|back|forward|tab.reload` on any tab. `js` is unclassified: `evaluate(code)` refuses unless the caller passes `{ effect, reason }`; a `read_only` claim is additionally checked against a static denylist (`=` assignment to `location|document.cookie|.value|.checked`, `.submit(`, `.click(`, `dispatchEvent(`, `fetch(`, `XMLHttpRequest`, `localStorage|sessionStorage|indexedDB`, `history.`); a hit refuses with `read_only_violation` and tells the caller to declare `mutating`. The explore probes at `surf-explore-operation.ts:322-335` are declared read_only in code and pass the denylist.
- Config `receipts.dir` in the kernel config schema `src/core/config.ts` (operator decision D4; default `.test-capabilities/receipts`, resolved against the config file's directory for `test` and `surf plan|apply`, against `--dir` for `heal` and against cwd for `init` and `replacement-validation`, which run without a config; revised by adjudication: claim 47; the submit-gate packet's `surf.receipts.dir` folds into this one key). The ledger writes through the `ReceiptStore { append, list(filter) }` interface, whose v1 implementation is files `<receipts.dir>/<run_id>/<receipt_id>.json` (revised by adjudication: claim 6; mode 0600, through the kernel artifact writer `src/core/artifacts.ts`, lifted from `heal-operation.ts:108-136` and extended with fsync on file and directory, artifact kinds, symlink refusals and `list(kind, filter)`; revised by architecture review: A9). The interlock is per store, and it fails closed: a mutating operation refuses with `mutation_receipts_ephemeral` when `receipts.dir` resolves inside an ephemeral workspace (a CI job workspace, `$TMPDIR`, a disposable worktree) unless the operator sets `receipts.ephemeral: true`, and that flag is recorded in every receipt written under it (operator decision D5). Ledger writes are framework bookkeeping, not steps: they carry no effect class and no receipt of their own, exactly as the dry-run artifacts do today. A mutating operation whose `receipts.dir` is not writable refuses before any step with `mutation_receipt_write_failed`; read-only operations never touch it.
...
- Revised by architecture review (A7, A8): the kernel boundary is `Adapter.invoke(step, ctx) → RawResult`, with `spawn-step.ts` as one transport (revised by adjudication: claims 2, 21, 37; the a11y channel's `/json/*` calls are an HTTP transport and the fake's function call a third); `runStep` is the only caller of `Adapter.invoke` for mutating steps, and the contract test that greps `src/` for `spawn(`/`spawnSync(` outside `spawn-step.ts` stays as the transport check. Browser steps run over the kernel `Session` interface (open, gate, `step(EffectStep)`, `observe(name, readOnlyStep)`, close in `finally`; revised by adjudication: claim 22), implemented by the surf adapter as an owned tab; explore, plan, apply, the frame diagnosis and the a11y observer are step lists over that session, so every browser step is an `EffectStep` and yields a `ResultOutcome` without special casing.
- Revised by architecture review (A13, Q1): the operator config key `mutation.allowOrigins: []` (default empty) is consulted by every `mutating/target` step whose subject is a web origin, the submit gate's `surf apply --submit` and the Bombadil agent alike; an origin outside it refuses before spawn with `mutation_origin_not_allowed`, naming the key.
...
```json
{
  ...
  "details": { "plan_id": "…", "mode": "fill|submit", "fields": [], "submit": {}, "surf_calls": [] }
}
```

`details` is the adapter-specific payload; the submit-gate packet's former `surf.receipt` fields live there and its outcome vocabulary is this one (`ok` is `applied`; a refusal before any act is an envelope error, not a receipt) (revised by architecture review: A1). Files under `receipts.dir` are 0600 and may carry values and page text; the envelope copies (`mutations[]`, `test --json`) carry hashes, codes, refs and counts only and name the artifact path (A10).

Envelope changes (additive): every `CliOperationResult` member gains `effect: EffectDeclaration` and `mutations: MutationReceipt[]` (empty for read-only runs); `TestResult` gains optional `mutations?: MutationReceipt[]` like `observations`. `heal` gains `--receipt-output <file>` (apply mode only, written through `writeJsonArtifactAtomically`, `heal-operation.ts:108-136`) with `artifact_kind: "test-capabilities.heal.receipts"`: this is the *aggregate export* of the run's receipts, not their only home; the per-receipt files in `receipts.dir` exist whether or not it is passed. `heal` and every later mutating operation also gain `--supersede-receipt <receipt_id>` (apply mode only) to reset an in-doubt interlock after the operator has inspected the subject. `appliedCount` becomes the count of receipts with `outcome: "applied"`. When a mutating operation throws, the thrown `MutationError` (a `FrameworkError { code, details }`; revised by architecture review: A6) carries `.receipts`; in text mode the CLI prints one line per receipt (`receipt_id outcome subject`) before the `[code]`-suffixed error, under `--json` the receipts land in `error.details.receipts` of the `{"error": {code, message, details}}` envelope, so an `unknown` outcome is never silent. Every `mutation_outcome_unknown` and `mutation_replay_refused` message carries the exact inspection hint and the exact `--supersede-receipt <receipt_id>` invocation (A16).

Error codes: `effect_unclassified`, `effect_declaration_invalid`, `mutation_retry_refused`, `mutation_replay_refused` (same run, or an in-doubt receipt on disk; the message carries `receipt_id`, `outcome`, `subject`), `mutation_outcome_unknown`, `mutation_receipt_write_failed` (the `attempting` receipt did not reach disk; the step is not run), `precondition_failed`, `read_only_violation` (static, before the run), `read_only_violation_observed` (dynamic, after an attempt), `owned_tab_required` (a target-affecting surf command without `--tab-id` on a tab this run created), `mutation_origin_not_allowed` (a `mutating/target` step whose web-origin subject is absent from `mutation.allowOrigins`; revised by architecture review: A13), `mutation_receipts_ephemeral` (operator decision D5). All framework codes are carried by `FrameworkError { code, details }` and registered as `EFFECT_ERROR_CODES` in `src/core/error-codes.ts` (A6).
...
- `SurfFlowBuilder` is deleted (revised by architecture review: A3; submit-gate D8): it held ambient authority to replay every click and its remaining `goto`/`wait`/`screenshot` steps have no test value (`goto` is mutating/target under the map above). `SurfClient` is deleted with it (revised by adjudication: claim 36; operator decision D2): the library's browser surface is the kernel `Session` interface (read steps, `plan`, `apply`; `evaluate(code, { effect, reason })` is a declared step on it), exported from `src/index.ts` at 0.4.0 with `consumer:smoke` proof; no facade. The removals land before any ratchet floor is measured (claim 13).
...
- Bombadil is one mutating/target step per agent run with `subject` = the origin of `targets.web` and `intent` = "bounded fuzz"; it runs only when that origin is in `mutation.allowOrigins`, otherwise the agent refuses before spawn with `mutation_origin_not_allowed` (revised by architecture review: A13, Q1; existing Bombadil configs get a refusal naming the key, `demo`/`init` are cli-tester only and unaffected); its trace path (`orchestrator.ts:1865-1867`) is the receipt evidence and its `verify`, read as typed facts (trace presence, exit contract), never as `looksLikeViolation`'s regex over stdout (revised by adjudication: claim 46): a run that produced a trace is `applied` whatever its exit (the effects are the fuzzer's purpose), a run without a trace is `unknown`. Bombadil's internal clicks are not individually receipted.
...
3. Fake-surf scenario, extend `tests/fixtures/fake-surf.mjs` and `tests/helpers/fake-surf.mjs` (`calls.log` records every argv, helper `:40-42`): with `failOn: ["extract"]` (transient once) the log shows two `extract` calls and `links.attempts: 2`; with `failOn: ["type"]` a direct `SurfClient.type(text, { selector })` call appears exactly once in the log, the receipt is `failed`, and a second call with the same key is refused (revised by architecture review: A3). The fake learns its shapes from `tests/fixtures/captures/`, never from prose (A17). Add `FAKE_SURF_HANG_ON` so a mutating `js` can time out and produce `unknown`; while it hangs, the CLI is killed with SIGINT and the rerun with the same key must be refused by the on-disk receipt. A read_only `js` probe whose fake answer carries an `href` outside the accepted set -> `read_only_violation_observed`, exactly one call in `calls.log`.
...
- Answered by architecture review (A5): `executeCliOperation` mints `RunContext { runId, startedAt, config, receiptsDir, ledger }` and passes it to `execute`; `SurfAgent` passes the orchestrator's context to the explore operation instead of re-entering `executeSurfExploreOperation` (`orchestrator.ts:2096`), so nested operations share one run and one ledger; the a11y channel derives its session name from `runId`.
...
- 2026-09-07, revised by architecture review (A1, A9, A10): one receipt kind for the whole framework; the submit-gate packet's receipt, directory and outcome vocabulary fold into this one, adapter payload under `details`; one kernel artifact writer with fsync; on-disk receipts may carry values, envelope copies carry hashes and codes only.
- 2026-09-07, revised by architecture review (A3): every `SurfFlowBuilder` claim withdrawn; the builder is deleted, not ledger-wrapped; `SurfClient` remains a public facade over `BrowserSession` (Q2, 0.4.0).
- 2026-09-07, revised by architecture review (A4): a mutating step left `unknown` has `ResultOutcome.basis: "indeterminate"`; the healer and the report treat it as not-a-target-fault.
- 2026-09-07, revised by architecture review (A5, A7, A8): `RunContext` is minted by the kernel; `spawnStep` is the one process boundary and `runStep` its only mutating caller; browser steps run over `BrowserSession`.
- 2026-09-07, revised by architecture review (A6, A16): errors are `FrameworkError` with a registered code, `[code]` in text mode and the surf-shaped JSON envelope under `--json`; receipt lines have a typed home in `error.details.receipts`; refusal messages carry the exact resolving command.
- 2026-09-07, revised by architecture review (A13, Q1): `mutation.allowOrigins` gates every mutating/target step with a web-origin subject, Bombadil included; the fuzzer is not exempt.
- 2026-09-07, revised by adjudication (claims 2, 21, 22, 37): the boundary is `Adapter.invoke`, spawn is one transport, and `Session` is a kernel interface the surf adapter implements; `runStep` is the only mutating caller of `invoke`.
- 2026-09-07, revised by adjudication (claim 36; operator decision D2): `SurfClient` is deleted with the builder; `Session` is the exported browser surface at 0.4.0; the removals precede the ratchet floor (claim 13).
- 2026-09-07, revised by adjudication (claims 6, 47; operator decisions D4, D5): `ReceiptStore` is an interface with files as v1; `receipts.dir` and `mutation.allowOrigins` come from the kernel config schema with a per-operation default; an ephemeral store is refused unless `receipts.ephemeral: true`, recorded in every receipt.
- 2026-09-07, revised by adjudication (claims 46, 50; operator decision D1): Bombadil's `verify` reads typed facts; `effect.reason` is rendered in every envelope; quantum and prediction stay as parked `read_only` code that never produces target evidence or touches a verdict.
