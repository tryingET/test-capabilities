...
- AK-native direction now records the current strategic frame and completed propagation-linkage wave; the remaining gap is keeping direction references synchronized with product posture without making docs a queue mirror.
...
3. Keep AK-native direction (`SF1`, completed `IW1`/`IW2`/`IW3`/`IW4`) linked to concrete tasks/evidence as the calibrated diagnosis frontier advances instead of reviving handoff files or markdown micro-plans.
