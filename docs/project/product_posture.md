...
| Intelligence layer | Finding correlation exists inside the orchestrator, supported agents emit `observation.v1` diagnostic events, and the orchestrator can add non-authoritative component-level semantic synthesis, suite-level observation correlation, deterministic calibrated `root_cause` observations for independent same-component failed-or-errored observed evidence from at least two sensors with same-class agreement, and non-authoritative `propagation` observations when dependent components both have high-calibration root_causes linked by a plausible causal mechanism (e.g., api timeout cascading to web) across default or operator-configured topology edges; `root-cause:corpus` dogfoods CLI, API, Surf, selector/DOM, Bombadil/property ambiguity, and propagation synthesis guardrails with machine-readable coverage floors. Prediction, collective learning, and self-healing intelligence flags are unsupported. | Cognitive surfaces turn observations into correlation, synthesis, calibrated diagnosis, and eventually prediction while preserving budget and local/privacy constraints. | Root-cause observations are deterministic and evidence-bounded, but the corpus is still fixture-based and heuristic rather than empirically broad; propagation topology is configurable but remains a heuristic input, not causal proof; prediction and collective learning remain vision-level until backed by empirical/runtime proof. | A broader calibrated root-cause corpus has measurable inputs, deterministic tests, acceptance thresholds, and explicit privacy posture before any prediction promotion. |
...
3. Keep AK-native direction (`SF01`, completed `WW01`/`WW02`/`WW03`, current `WW04`) linked to concrete tasks/evidence as the multi-component propagation synthesis frontier advances instead of reviving handoff files or markdown micro-plans.
