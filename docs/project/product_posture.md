...
| Intelligence layer | Finding correlation exists inside the orchestrator, supported agents emit `observation.v1` diagnostic events, and the orchestrator can add non-authoritative component-level semantic synthesis plus suite-level observation correlation when correlation is enabled and multiple observations are present; prediction, collective learning, and self-healing intelligence flags are unsupported. | Cognitive surfaces turn observations into correlation, synthesis, and eventually prediction while preserving budget and local/privacy constraints. | Observation-native synthesis explains sensor meaning but does not yet infer calibrated root cause or predict future failure; prediction and collective learning remain vision-level until backed by empirical/runtime proof. | A first calibrated root-cause/prediction slice has measurable inputs, deterministic tests, acceptance thresholds, and explicit opt-in/privacy posture. |
...
- Observation synthesis is diagnostic rather than calibrated root-cause inference; future intelligence/autonomy slices need empirical proof and privacy/approval boundaries before they are promoted from aspiration to supported capability.
...
4. Promote the next cognitive slice from diagnostic semantic synthesis to calibrated root-cause inference across Surf, Bombadil, and CLI smoke before attempting prediction or autonomy.
