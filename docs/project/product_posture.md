...
| Orchestrator sensors | `cli-tester`, `surf`, and `bombadil` are implemented; Surf wraps the supported `surf explore` path, standardizes on Surf Go from env/workspace/`PATH`, verifies browser-state evidence after navigation, and fails closed on invalid explicit Surf Go repo env vars or unverified Surf output; Bombadil resolves from explicit env, built contrib checkout, repo-local binary, or `PATH`. Packed consumers intentionally treat Bombadil as an external binary requirement. | Sensor integrations become a small, typed parliament of real observers with normalized findings and evidence. | `api-fuzzer` remains unsupported; packed Bombadil consumers must provide an external Bombadil binary, and Surf consumers must provide a resolvable Surf Go runtime. | Supported sensors have deterministic fixtures/smokes, generated passport evidence, and packed-consumer proof for external binary requirements. |
...
- `cli-tester`, Surf-backed, and Bombadil-backed orchestrator execution are implemented and covered by contract/regression checks.
...
3. Keep AK-native direction (`SF1` / `IW1`) linked to concrete tasks/evidence as the next capability frontier advances instead of reviving handoff files or markdown micro-plans.
