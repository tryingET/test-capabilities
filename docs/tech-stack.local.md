...
- TypeScript validation and declaration-emitting package builds now use `tsgo` via `@typescript/native-preview`; the previous `tsc` fallback was removed after replacing the lone TypeScript compiler-API fixture with a static source-map fixture. Install without `--omit=optional` so the native platform package is present; build/type gates smoke-test `tsgo --version` before trusting the wrapper.
