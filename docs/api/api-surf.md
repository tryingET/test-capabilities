...
**Status at 0.4.0.** `SurfClient` and `SurfFlowBuilder` are no longer exported from the package root (operator decision D2): the class carried ambient browser authority into every consumer and its `parseSnapshot`/flow helpers were never live-verified. The public browser surface arrives as the kernel `Session` interface (`open`, `gate`, `step`, `observe`, `close`, always an owned tab closed in `finally`) later in this release line; until then the supported programmatic path is `executeCliOperation({ command: 'surf', action: 'explore' }, { url })`. The internal `src/integrations/surf-client.ts` was deleted in the same release line once the quality ratchet's never-imported rule showed that no runtime path reached it (last version: commit `30b0cbb`). The method reference below documents the mappings that class carried, for maintainers; treat it as the shape `Session` will carry, not as a consumer API.
