...
- `correlation: true`, including non-authoritative synthesis, suite correlation, and deterministic `root_cause` observations when same-component evidence has at least two independent observed current-run evidence units from at least two sensors
- `selfHealing: false`
- `prediction: false`
- `collective: false`
