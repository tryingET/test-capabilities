...
- when same-component evidence has at least two independent observed current-run evidence units from at least two sensors, the orchestrator can also emit `root_cause` observations with deterministic calibration metadata; derived observations do not count separately from their source findings, and the result identifies an evidence-bounded current failure class, not a forecast or probability claim
