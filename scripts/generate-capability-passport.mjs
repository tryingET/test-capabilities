...
capabilities.push(
  capabilityEntry({
    ...
    notes: bombadilPresent
      ? "Repo-local external/bombadil is one binary provider for the supported Bombadil agent in this checkout; a built source checkout referenced by TEST_CAPABILITIES_BOMBADIL_REPO can override it locally, while packed consumers still need TEST_CAPABILITIES_BOMBADIL_BIN, TEST_CAPABILITIES_BOMBADIL_REPO, or bombadil on PATH."
...
capabilities.push(
  capabilityEntry({
    ...
    notes:
      "Supported orchestrator agents emit observation.v1 diagnostic events for Surf coverage, Bombadil property exploration, and CLI smoke execution. When correlation is enabled, orchestrator runs can also include component-level semantic synthesis, suite-level observation correlation, deterministic calibrated root_cause observations for at least two independent same-component failed-or-errored observed current-run evidence units from at least two sensors that agree on the same failure class, and non-authoritative propagation observations when dependent components both have high-calibration root_causes and a bounded propagation link appears across default or operator-configured topology edges (e.g., api timeout paired with web runtime failure). Propagation observations carry low calibration by design, declare non-authoritative heuristic status in their evidence, and never make prediction or causality-overclaim language. The root-cause corpus dogfoods CLI, API contract/runtime/auth-boundary/network-connectivity/resource-exhaustion/configuration, Surf, selector/DOM, Bombadil/property ambiguity, same-component mixed-class suppression, linked finding/current-run evidence disagreement suppression, simultaneous component-scoped diagnoses, three-sensor agreement calibration, multi-component (Bombadil+CLI, Surf+CLI+API) fixtures, cli-to-api/cli-to-web/custom-topology propagation cases, and propagation synthesis guardrails including auth-boundary, network-connectivity, resource-exhaustion, and configuration-error no-propagation with machine-readable root-cause and propagation coverage floors. The runtime diagnostic corpus adds a narrow real cli-tester subprocess proof lane for single-sensor suppression, two-sensor command_resolution, two-sensor timeout_or_latency, same-component mixed-class suppression, correlation-disabled suppression, unique observation IDs, and no prediction/causal/repair-order wording. The public library exports the bounded root-cause vocabulary as RootCauseFailureClass plus ROOT_CAUSE_FAILURE_CLASSES, root_cause observations expose structured semantics.failureClass, propagation observations expose structured semantics.propagationLink, and the packed-consumer smoke proves calibrated root_cause and low-calibration non-authoritative propagation observations survive through the distributed library API. Observations explain sensor facts; findings and coverage remain pass/fail authority.",
  }),
);
  ...
  },
  {
    ...
    evidence: {
      tests: [
        ...
        "tests/a11y_cdp_contract.test.mjs",
    ...
    notes:
      "An optional, read-only second observation channel registered on Session.observe: Chromium's own accessibility tree of the tab the surf run already owns, read with Accessibility.getFullAXTree over the loopback CDP endpoint from the existing page target (no new target or tab) and from each out-of-process frame through its own flattened session, then detached. The producer was chosen by a measured series (AK #5915): surf's content-script tree missed 964 shadow-root buttons on MDN and gave non-accessible names (86% agreement on GitHub). The schema a11y-snapshot.v1 is the contract (channel chromium-ax-cdp). The artifact carries the rendered tree (controls, headings, landmarks, named images; wrappers collapsed; refs in document order), the {role, name} refs map, the sha256 digest, the role counts and semanticCoverage, written under receipts.dir/<runId>/ at 0600 while the envelope keeps the digest, the refs and the counts. openA11yLiveView gives evaluateA11yAssertion a read-only reader for visible, text and attr, in-frame too. The endpoint must be loopback and is refused before any request otherwise; the tab binds to exactly one target or refuses. Off by default. Live on Chromium (Agent) Chrome/153: GitHub releases 256 refs / 7308 bytes with the same digest across tabs and hours, MDN 1260 refs across 6 frames, page count unchanged, in-frame checks passed.",
  },
  {
